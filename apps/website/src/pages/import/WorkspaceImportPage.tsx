...
import { PlusIcon, TimerIcon } from "../../shared/ui/icons.tsx";
...
import { ArchiveImportSection } from "./ArchiveImportSection.tsx";
  ...
  async function handleArchiveUpload() {
    ...
    try {
      ...
      if (job.organization_id) {
        ...
        try {
          await updateWebSessionMutation.mutateAsync({ workspace_id: job.workspace_id });
        } catch (error) {
          toast.error(resolveImportErrorMessage(error) ?? t("workspaceSwitchFailed"), {
            duration: 6000,
          });
        }
      }
    ...
    <PageLayout title={t("import")}>
      <div className="mx-auto max-w-2xl space-y-5 p-5">
        ...
        <ArchiveImportSection
          ...
          isPending={createArchiveImportJobMutation.isPending}
          onArchiveChange={setSelectedArchive}
          onOrganizationNameChange={setOrganizationName}
          onUpload={() => void handleArchiveUpload()}
