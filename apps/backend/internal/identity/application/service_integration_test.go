...
func TestDeactivatedSessionsCannotResolveCurrentUserOrUpdateAccountState(t *testing.T) {
	service := newTestService()
	...
	if _, err := service.ResolveCurrentUser(context.Background(), auth.SessionID); !errors.Is(err, domain.ErrUserDeactivated) {
		t.Fatalf("expected current session lookup to fail for a deactivated user, got %v", err)
	}

	if _, err := service.UpdateProfile(context.Background(), auth.User.ID, domain.ProfileUpdate{
		FullName: "Blocked Rename",
	}); !errors.Is(err, domain.ErrUserDeactivated) {
		t.Fatalf("expected deactivated profile update to fail, got %v", err)
	}

	if err := service.UpdatePreferences(context.Background(), auth.User.ID, "web", domain.Preferences{
		DateFormat: "YYYY-MM-DD",
	}); !errors.Is(err, domain.ErrUserDeactivated) {
		t.Fatalf("expected deactivated preferences update to fail, got %v", err)
	}
}
