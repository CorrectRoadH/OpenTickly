---
title: セルフホスト
description: OpenTickly を自分のマシンにデプロイ — Docker Compose 一発で Web + API + Postgres + Redis を起動、5 分で稼働。Toggl API 互換 100%、公式クライアントからそのまま接続できます。
---
...
OpenTickly には 1 時間あたり 30 回の API レート制限がありません。つまり、Claude や Cursor のようなエージェントを思い切り使えます。[`toggl-cli`](/ja/docs/ai-integration) と組み合わせて、スキルパックをエージェント環境にインストールすれば、数分で AI にタイマーの開始・停止やプロジェクト整理を任せられます。
...
3. [`docker-compose.yml`](https://raw.githubusercontent.com/CorrectRoadH/OpenTickly/main/docker-compose.yml) の内容を貼り付ける
...
3. **docker-compose.yml を使用** を選び、[compose ファイル](https://raw.githubusercontent.com/CorrectRoadH/OpenTickly/main/docker-compose.yml) を貼り付ける
4. ウィザードを最後まで進めて完了
...
4. **Compose 内容** に [compose ファイル](https://raw.githubusercontent.com/CorrectRoadH/OpenTickly/main/docker-compose.yml) を貼り付ける
...
- [GitHub リポジトリ](https://github.com/CorrectRoadH/OpenTickly) — ソース、Issues、Discussions
- デプロイで問題に当たったら [Issues](https://github.com/CorrectRoadH/OpenTickly/issues) でお知らせください
