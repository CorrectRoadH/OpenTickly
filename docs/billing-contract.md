...
将 billing / subscription / invoice / payment / quota 的公开兼容合同单独定义，避免它们混杂在总 PRD 中。

## 当前结论

- billing 是首版兼容范围的一部分，不是后续可选功能。
- 即使底层计费实现不同，对外公开对象、关键状态和关键错误语义也必须兼容。
- 本文档只定义公开兼容合同，不定义内部支付、开票、清结算实现。
...
- 但对外暴露的对象结构、关键状态、关键流程和关键错误语义必须兼容。
- 兼容目标是“现有调用方仍把 OpenToggl 视为 Toggl 风格的 billing surface”。
...
- 当某项能力需要升级套餐才能使用时，应优先沿用兼容的错误码与错误语义。
- 兼容目标包括：
  - 返回时机
  - 返回码
  - 关键错误信息类别
...
- customer 是订阅合同的关联对象。
- 如果某些内部字段不存在，可以不公开，但兼容字段必须稳定返回或按兼容语义为空。
...
- 未找到、无权限、对象不属于该组织/工作区时，应优先返回兼容错误码与兼容错误类别。
...
如果内部系统状态更多，可以内部扩展；但对外不得削弱兼容状态表达。
...
- 即使 OpenToggl 给用户更宽松额度，也应保留兼容的 quota 表达方式。
...
- 自托管版至少要能返回兼容的 subscription/customer/invoice/quota 对象和状态。
- 自托管版是否支持真实在线扣款，留作后续实现决策；但这不影响公开合同先行定义。

## 待确认项

- setup_intent 与真实支付网关绑定的最小兼容语义
...
该文档用于支撑 `docs/prd.md` 中 Billing / Subscription / Invoice 章节。
