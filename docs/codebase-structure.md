...
`webhooks` 保持顶层模块，不并入通用 `integrations`，因为它有独立兼容合同和完整运行时语义。
...
- 兼容 API DTO 和前端展示模型可以不同，不要求前端直接复用后端内部结构。
...
- 若某个部署模型下不存在真实 subscription 创建动作，`billing/application` 仍需要返回兼容的默认商业状态
...
本节只定义实现顺序建议，不代表首版范围裁剪。首版正式版本的范围仍以 `docs/prd.md` 中已承诺的完整兼容能力为准。
