...
将 `Reports API v3` 的统计口径、freshness、重建语义和导出合同单独定义，避免“字段看起来一样但结果解释不一样”。

## 当前结论

- Reports 是独立产品面，不是 Track API 的附加查询层。
- 这里定义的重点包括结果口径，而不只是接口存在。
...
以下内容官方公开资料没有完全写透，OpenToggl 需要补内规则，但不能假装官方已明示：
...
- 统一以时间记录事实为源。
- 存储与内部计算可以 UTC 为主。
- 对外切日、切周、展示和聚合必须遵守固定时区语义。

### 舍入口径

- `rounding` 与 `rounding_minutes` 属于公开业务语义。
- 舍入影响不仅是 UI 显示，还可能影响聚合和导出。
- 如果官方公开定义要求聚合前舍入或聚合后舍入，必须固定，不允许报表间自相矛盾。
...
- 当前默认优先级定义为：
  ...
  - user/member rate
  - workspace 默认 rate
- 上层 rate 为 `null` 或未设置时，必须显式回落到下一层。
- 该优先级必须在 detailed、summary、weekly、profitability、export 中保持一致。
- 历史 rate 变更不得默默改写已审批、已导出、已共享或已结算期间的历史结果。
...
- 被停用或删除主体所产生的历史 time entries，默认仍属于报表事实的一部分，不因主体失活而从历史统计中消失。
- 归档或删除的 project / user / client / tag，如果其历史 time entries 仍在报表时间范围内，报表应继续反映这些事实，而不是偷偷改算到 `No Project` 或直接丢弃。
- 对象在报表中的显示标签可以变化，但历史 duration、cost、revenue、profit 不得因对象状态变化而被静默抹除。
...
官方当前至少已经把“报表过大无法导出”做成了公开错误语义，因此 OpenToggl 也需要把这类边界视为公开合同的一部分，而不是内部异常。
...
- 共享导出权限与在线查看权限必须自洽。
- 访问者可以在执行 shared report 时覆盖查询参数，但覆盖后的参数不得隐式回写 saved definition。
- 只有显式保存或更新操作，才允许修改 saved/shared report 的持久化查询定义。
- public / private 的权限差异必须同时作用于：
  - 在线读取
  - CSV/PDF/XLSX 导出
  - 参数覆盖后的执行结果
- owner 不再活跃、report 被禁用、token 被撤销时，shared report 必须以固定公开错误语义失败，而不是回退成匿名可访问。
...
- owner 不再活跃于该 workspace 时，shared report 可返回 `403`
...
该文档用于支撑 `docs/product/reports-and-sharing.md`。

逐端点矩阵见 `docs/contracts/reports-endpoint-matrix.md`。
